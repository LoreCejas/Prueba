...
export class NewPartnersComponent implements OnInit {
  ...
  coopSelected(coop) {
    ...
    if (this.selectedCoop.tipo != 2) {
      this.form.get('cuentaCorriente').setValidators(null);
    } else {
      this.form.get('cuentaCorriente').setValidators(Validators.required);
    }
  }
  ...
  create(obj: any) {
    ...
    const newData: AddPartnersRequest = {
      ...
      cuentaCorriente: obj.cuentaCorriente.toString(),
      esccc: true,
    }
    this.service.addPartners(newData).subscribe((result) => {
      ...
      this.errors = error;
      this.loginError = true;
      this.errorService.sendNotification("error", "add_socios_error");
    });
  }
  ...
  save(obj: any) {
    ...
    const data: UpdatePartnersRequest = {
      ...
      cuentaCorriente: obj.cuentaCorriente.value? obj.cuentaCorriente.value : 0
    }
    this.service.updatePartners(data).subscribe((result) => {
      ...
      this.errors = error;
      this.loginError = true;
      this.errorService.sendNotification("error", "update_socios_error");
    });
  }
